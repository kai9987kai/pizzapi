...
# autodoc has to be able to import pizzapy, which lives one level up from
# this directory. Without this, every automodule directive silently documents
# nothing.
import io
import os
import re
...
# The docstrings are written in Google style ("Args:", "Attributes:").
...
# built documents.
#
# Read straight out of the package, so the docs cannot fall behind the code.
with io.open(os.path.join(os.path.dirname(__file__), os.pardir, 'pizzapy',
                          '__init__.py'), encoding='utf-8') as _f:
    _version = re.search(r"^__version__ = '([^']+)'", _f.read(), re.M).group(1)
